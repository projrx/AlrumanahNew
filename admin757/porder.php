...
<head>
  ...
  if(isset($_POST['action'])){
    ...
    $date = date('Y-m-d');
    $sql = "UPDATE shop SET `status` = 'enroute', `edate` = '$date' WHERE `id` = '$oid' ";

    mysqli_query($con, $sql) ;
    ($msg=mysqli_error($con));
    if(empty($msg))  $msg="Order Dispatched Successfully !";
  ...
  <div class="row">
    ...
    <div class="col-md-12">
      <div class="widget-container fluid-height clearfix overflow">
        <div class="heading">
                      <i class="fa fa-tags"></i>Orders Requiring Admin Approval and Ready for Enroute.
                    </div>
        <div class="widget-content padded clearfix">
          ...
          <tbody>
            ...
            <?php 
            $rows =mysqli_query($con,"SELECT * FROM shop where status='pending'  Order By id desc LIMIT $glimit " ) or die(mysqli_error($con));
            ...
            while($row=mysqli_fetch_array($rows)){
              ...
              while($rowx=mysqli_fetch_array($rowsx)){
                ...
                <tr class="orow" onclick="window.location='porders-<?php echo $oid ?>'">
                 ...
                 <td><form action="" method="POST">
                  <button type="submit" name="action" value="<?php echo $oid ?>" class="btn"> Dispatch </button>
                </form>
